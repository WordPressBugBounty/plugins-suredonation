<?php
/**
 * Charitable migration orchestrator.
 *
 ...
 * `suredonation_import_charitable_phase_mappers` so the Pro plugin can plug
...
class Importer {
	 ...
	 *
	 * @return array{has_data:bool, charitable_version:?string, counts:array<string,int>, gateway_breakdown:array<int,array<string,mixed>>}
	 * @since  1.0.0
	 */
	public function get_counts() {
		...
		// When the Charitable tables were never created, short-circuit the
		// counts that query them (donations / standalone donors) so we don't
		// emit "table doesn't exist" DB errors during the pre-flight read.
		$has_data = $source->has_charitable_data();

		return [
			'has_data'           => $has_data,
			'charitable_version' => $source->get_charitable_version(),
			'counts'             => [
				'campaigns'         => $source->get_campaign_count(),
				'donations'         => $has_data ? $source->get_donation_count() : 0,
				'subscriptions'     => $source->get_subscription_count(),
				'standalone_donors' => $has_data ? $source->get_standalone_donor_count() : 0,
			],
	 ...
	 * @return array<string, mixed>|false Updated progress payload, or false if the session is missing or no longer running.
	 * @since  1.0.0
	 */
	...
	private function get_phase_mapper( $phase ) {
		 ...
		 * @since 1.0.0
		 */
		$mappers = apply_filters( 'suredonation_import_charitable_phase_mappers', $mappers );
	 ...
	 *
	 * @param  string               $import_id Session UUID.
	 * @param  array<string, mixed> $progress  Progress payload after this batch.
	 * @return void
	 * @since  1.0.0
	 */
	private function fire_batch_complete( $import_id, $progress ) {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'suredonation_import_charitable_batch_complete', $import_id, $progress );
	}
}
